...
class Admin_model extends CI_Model
{
    ...
    function get_stat_applicant()
    {
        ...
        $query = $this->db->query("SELECT	id_job,posisi,
    ...
    function get_applicant($x, $by)
    {
        if ($by == 'by_status') {
            if ($x == 'all') {
                $where = "";
            } else {
                $where = "WHERE status='$x'";
            }
        } elseif ($by == 'by_job') {
    ...
    {
        $q = "SELECT    m1.id_member,m1.username,m1.nama,m1.no_hp,m1.status,m1.tgl_reg,m1.kota,m1.alamat,m1.id_location,m1.level,m1.no_rekening,m1.nama_rekening,m1.kode_bank,
