...
class Auth extends CI_Controller
{
	...
	public function act_login()
	{
		...
		$member     = $this->db->query("SELECT * FROM ilufa_master_member WHERE password = '" . $password . "' AND (phone= '" . $username . "' OR email = '" . $username . "')");
		$member_cek	= $member->num_rows();
		$member_row	= $member->row();

		$branch     = $this->db->query("SELECT * FROM ilufa_master_branch WHERE password = '" . $password . "' AND (id_branch= '" . $username . "' OR email = '" . $username . "'  OR phone = '" . $username . "')");
		$branch_cek	= $branch->num_rows();
		$branch_row	= $branch->row();

		$admin		= $this->db->query("SELECT * FROM ilufa_master_admin WHERE password = '" . $password . "' AND (username = '" . $username . "' OR email = '" . $username . "')");
		...
		if ($member_cek == 1) {
			...
			$data_login = array(
				'log_id'        => $branch_row->id_branch,
				'log_user'      => $branch_row->id_branch,
				'log_name'      => $branch_row->branch_name,
				// 'log_email'  	=> $branch_row->email,
				'log_level'     => "store",
				'log_admin'     => FALSE,
				'log_valid'     => TRUE
			);
			...
			$this->session->set_flashdata("report", "<h5>login Gagal</h5> Username atau Password yang anda masukkan salah ...");
